...
###
# Run this as a background job, it captures the System CPU, Memory, Disk utilizations and
# Docker container resource utilization and logs the data to Mongo DB for the latest values
# and to Influxdb for the historic Time series data.
...
def write_sys_resources_to_dbs( hdl_dict, mdb, mongodb_name, influx, influx_db_name ):
    ...
    mdb.update_record( psm_resource_collection, { 'psm-cluster-name': psm_cluster_name }, { 'psm-cluster-name': psm_cluster_name, 'cpu-info': json.dumps(cpu_dict), 'mem-info': json.dumps(mem_dict), 'disk-info': json.dumps(disk_dict), 'proc-mem-distribution-info': json.dumps(mem_dist_dict) } )
    # Write to influx
...
while True:
    ...
    mdb.update_record( docker_res_collection, { 'psm-cluster-name': psm_cluster_name },  { 'psm-cluster-name': psm_cluster_name, 'docker-mem-info': json.dumps(docker_res_dict) } )


    log_dict = sys_utils.get_cluster_log_summary_dict( hdl_dict )
    mdb.update_record( psm_log_summary_collection, { 'psm-cluster-name': psm_cluster_name }, { 'psm-cluster-name': psm_cluster_name, 'log-info': json.dumps(log_dict) } )


    go_profile_dict = get_go_allocs_profile_dict( hdl_dict )
    mdb.update_record( go_profile_collection, { 'psm-cluster-name': psm_cluster_name }, { 'psm-cluster-name': psm_cluster_name, 'alloc-info': json.dumps(go_profile_dict) } )


    proc_mem_dist_dict = sys_utils.get_cluster_process_memory_distribution_dict( hdl_dict )

    mdb.update_record( proc_mem_dist_collection, { 'psm-cluster-name': psm_cluster_name }, { 'psm-cluster-name': psm_cluster_name, 'mem-dist': json.dumps(proc_mem_dist_dict) })
